...
class Network(BaseNetwork):
    ...
    def p_mean_variance(self, y_t, t, clip_denoised, y_cond):
        noise_level = self.gammas[t].repeat(y_t.shape[0], 1).to(y_t.device)
    ...
    def forward(self, y_0, y_cond=None, mask=None, noise=None):
        ...
        if mask is not None:
            x_recon = self.denoise_fn(torch.cat([y_cond, x_noisy*mask+(1.-mask)*y_0], dim=1), sample_gammas)
            loss = self.loss_fn(mask*noise, mask*x_recon)
        else:
            x_recon = self.denoise_fn(torch.cat([y_cond, x_noisy], dim=1), sample_gammas)
